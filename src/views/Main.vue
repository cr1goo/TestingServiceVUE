  ...
  <el-card class="box-card" style="min-width: 1000px">
    <div slot="header" class="clearfix">
      ...
      <div style="float: right; ">
        <el-button
          type="text"
          style="padding: 3px 0;"
          @click="changePassFormForm.show = true"
        >Смена пароля</el-button>
    ...
    <el-radio-group v-model="tests.show">
      <el-radio-button label="available">
        <i class="el-icon-question"></i> Доступные
      </el-radio-button>
      <el-radio-button label="completed">
        <i class="el-icon-success"></i> Завершенные
      </el-radio-button>
    </el-radio-group>
    ...
    <el-dialog title="Смена пароля" :visible.sync="changePassForm.show" width="500px">
      <el-input
        ...
        v-model="changePassForm.old"
        style="margin-bottom: 10px;"
      ></el-input>
      <el-input
        placeholder="Новый пароль"
        v-model="changePassForm.new"
        style="margin-bottom: 10px;"
      ></el-input>
    ...
    <el-dialog title="Добавить тест" :visible.sync="newTestForm.isShow" width="500px">
      <el-select
        v-model="newTestForm.data.discipline"
        placeholder="Дисциплина"
        style="margin-bottom: 10px; width: 100%;"
      ></el-select>
      <el-input
        placeholder="Название"
        v-model="newTestForm.data.name"
        style="margin-bottom: 10px; width: 100%;"
      ></el-input>
      <el-input
        placeholder="Описание"
        v-model="newTestForm.data.description"
        style="margin-bottom: 10px; width: 100%;"
      ></el-input>
      <el-select
        v-model="newTestForm.data.type"
        placeholder="Тип"
        style="margin-bottom: 10px; width: 100%;"
      ></el-select>
      <el-date-picker
        v-model="newTestForm.data.time"
        type="datetime"
      ...
      <center>
        <el-upload
          ...
          :limit="1"
        >
          <el-button slot="trigger" size="small" type="primary">Выберите файл</el-button>&nbsp;или
          <el-button
            type="warning"
            size="small"
            @click="newTestForm.manualInput = true"
          >Добавить вручную</el-button>
      ...
      <span slot="footer" class="dialog-footer">
        <center>
          <el-button
            type="primary"
            icon="el-icon-search"
            @click="newTestForm.previewDialog = true"
    ...
    <el-dialog title="Ручное добавление" :visible.sync="newTestForm.manualInput" width="800px">
      <el-button
        ...
        style="margin-bottom: 10px;"
        icon="el-icon-plus"
      >Добавить вариант</el-button>
      <el-tabs
        type="border-card"
        v-model="newTestForm.currentTestIndex"
        closable
        @edit="delVariant"
      >
        <el-tab-pane
          v-for="(variant, varid) in newTestForm.data.tests"
          :key="varid"
          :label="'Вариант '+(varid+1)"
        >
          <el-button
            type="success"
            size="small"
            icon="el-icon-plus"
            @click="addNewQuestion(variant.questions)"
            style="margin-bottom: 10px;"
          >Добавить вопрос</el-button>
            ...
            <el-collapse-item>
              <template slot="title">
                <el-button
                  ...
                  size="small"
                  icon="el-icon-delete"
                  @click="delQuestion(newTestForm.activeVariant, questid)"
                />
                &nbsp;
              ...
              <template v-for="(answer, ansid) in question.answers">
                <el-input
                  v-model="question.answers[ansid].value"
                  :key="ansid"
                  style="margin-bottom: 10px; width: 100%"
                  placeholder="Содержание ответа"
                >
                  <template slot="prepend">
                    <el-checkbox v-model="answer.correct" :key="ansid"/>
                  </template>
                  <template slot="append">
                    <el-button
                      ...
                      size="medium"
                      icon="el-icon-delete"
                      @click="delAnswer(newTestForm.activeVariant, questid, ansid)"
                      :key="ansid"
                    />
                  </template>
                </el-input>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button
          type="success"
          @click="newTestForm.manualInput = false"
          icon="el-icon-check"
        >Сохранить</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog title="Предпросмотр" :visible.sync="newTestForm.previewDialog" width="800px">
      <el-tabs type="border-card">
        ...
        >
          <div
            v-for="(question, questid) in variant.questions"
            :key="questid"
            style="margin-bottom: 10px; border-bottom: 1px dashed #DCDFE6; padding-bottom: 10px;"
          >
            <div>
              <b>{{ 'Вопрос '+(questid+1)+'. '+question.title }}</b>
              <div
                v-for="(answer, ansid) in question.answers"
                :key="ansid"
              >{{ (ansid+1)+'. '+question.answers[ansid].value}}</div>
              <div>
                Верный(е) ответ(ы):
                <span v-for="(answer, ansid) in question.answers" :key="ansid">
                  <b v-if="question.answers[ansid].correct">{{ (ansid+1) }}</b>
                </span>
              </div>
            </div>
          </div>
